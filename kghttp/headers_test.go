...
func TestFieldLineParse(t *testing.T) {
	...
	t.Run("valid capital header key", func(t *testing.T) {
		headers := NewHeaders()
		data := []byte("Host: localhost:42069\r\n\r\n")
		n, done, err := headers.Parse(data)
		require.NoError(t, err)
	...
	t.Run("valid header multiple values", func(t *testing.T) {
		headers := Headers{"set-person": "kg-loves-ts"}
		data := []byte("Set-Person: kg-loves-go\r\n\r\n")
		_, _, err := headers.Parse(data)
		require.NoError(t, err)
		require.NotNil(t, headers)
		assert.Equal(t, "kg-loves-ts, kg-loves-go", headers["set-person"])
	})
